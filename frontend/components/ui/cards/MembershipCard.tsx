...
function MembershipCard({
  ...
  const parseParagraphs = (features: string): string[] => {
    if (!features || !features.trim()) return [];
    try {
      const listItems = features.match(/<p[^>]*>(.*?)<\/p>/g) || [];
      return listItems.map((item) =>
        item.replace(/<p[^>]*>(.*?)<\/p>/, "$1").trim()
      );
    } catch (error) {
  ...
  const itemData = useMemo(() => parseFeatures(features), [features]);
  ...
  return (
    <>
      <div className="relative rounded-2xl sm:rounded-3xl lg:rounded-3xl overflow-hidden w-full h-full shadow-md p-4 sm:p-6 lg:p-6 flex flex-col justify-between border-solid border-2 border-gray-50 bg-white bg-[url(/traditional_ring.png)]">
        <div className="space-y-6 sm:space-y-8 lg:space-y-8">
          {/* Reserve space for the badge to keep heading alignment consistent */}
          <div
            style={{
              minHeight: "40px",
              display: "flex",
          ...
          >
            {isPopularBool ? (
              <span className="bg-yellow-300 px-4 py-2 rounded-full  text-blue-500 text-sm hover:bg-yellow-400 hover:text-blue-400">
                Most popular
              </span>
            ) : (
              <span
          ...
          <p className="text-center text-gray-400 space-y-0 p-0 m-0 text-xs sm:text-sm lg:text-sm font-normal">
            {parseParagraphs(features)}
          </p>
          <hr className="h-px bg-gray-200 border-0 dark:bg-gray-700"></hr>
          <div className="space-y-2 sm:space-y-4 lg:space-y-2">
            {features && features.trim() ? (
              <div className="text-sm sm:text-base lg:text-base space-y-1 sm:space-y-0 lg:space-y-0 text-gray-500">
                <ul className="space-y-0 md:space-y-2 lg:space-y-2">
                  {itemData.map((item, i) => (
                    <li key={i} className="text-gray-500 text-sm flex">
                      <span>
                        <svg
              ...
              <ul className="list-disc list-inside mt-2 text-sm sm:text-base lg:text-base space-y-1 sm:space-y-0 lg:space-y-0 text-left text-gray-700">
                ...
                <li>
                  ...
                  Preferential seating at check-in
                </li>
